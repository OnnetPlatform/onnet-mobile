...
export const Glassmorphism: React.FC<{ step: number; onAnimationEnd(): void }> = ({
  step,
  onAnimationEnd,
}) => {
  ...
  useEffect(() => {
    if (step === 1) {
      runSpring(white_circle_x, { from: white_circle_x.current, to: width - (step * width) / 2 });
  ...
  return (
    <Canvas style={StyleSheet.absoluteFillObject}>
      ...
      <Circle r={blue_circle_radius} cx={blue_circle_x} cy={blue_circle_y} color={colors.blue} />
      <Circle
        ...
        color={colors.yellow}
      />
      <Circle r={white_circle_radius} cx={white_circle_x} cy={white_circle_y} color={color} />
